...
then
  ...
  then
    ...
    pfa_id=`sbatch -J ${DATASET_NAME}ParseForAnalysis ${RESOURCE_FLAG} --array=1-${NUM_FILES} --time=${PREPROCESS_MAX_RT} --mem=${PREPROCESS_VMEM} -D ${CODE_DIR} -o ${LOG_DIR}/glm/${DATASET_NAME}ParseForAnalysis_%A_%a.out -e ${LOG_DIR}/glm/${DATASET_NAME}ParseForAnalysis_%A_%a.err --depend=afterok:${a_id} analysis/parseForAnalysis.sh SLURM ${ALIGN_PARDIR} ${DATASET_NAME} ${MODE} | awk '{print $4}'`
    sbatch -J PFA${DATASET_NAME} ${ANALYSIS_RESOURCE_FLAG} --array=1-${NUM_FILES} --time=${PREPROCESS_MAX_RT} --mem=${PFA_VMEM} -D ${CODE_DIR}/analysis -o ${LOG_DIR}/glm/${DATASET_NAME}PredictJunctions_%A_%a.out -e ${LOG_DIR}/glm/${DATASET_NAME}PredictJunctions_%A_%a.err --depend=afterok:${pfa_id} analysis/predictJunctions.sh SLURM ${ALIGN_PARDIR} ${DATASET_NAME} ${MODE} ${REPORTDIR_NAME}
  fi
