...
source ../analysis/depends_bt2.sh  # load bowtie module
...
DATASET_NAME=$3
MODE=$4
INDEX=$5  # name of the bowtie index passed to bowtie2 call
ALIGN_OUTDIR=$6  # name of directory under orig to output alignment files, also used in output file names
FASTA=$7 # name of fasta file under index to use

source ../analysis/depends_sampleInfo.sh  # get sample-specific variables from TASK_DATA_FILE
...
if [[ "$MODE" = *bam* ]]
then
  source ../analysis/depends_bam.sh  # load samtools
