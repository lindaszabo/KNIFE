...
module load python/2.7.5  ## SPECIFIC TO OUR CLUSTER, to use more recent version of python than the default
...
then
  ...
  if [ $# -eq 7 ]
  then
    OPT_ARGS=`echo "-j ${7}"`  # id suffix was passed
  elif [ $# -eq 8 ]
  then
    OPT_ARGS=`echo "-a1 ${7} -a2 ${8}"`
  elif [ $# -eq 9 ]
  then
    OPT_ARGS=`echo "-j ${7} -a1 ${8} -a2 ${9}"`
  fi
  ...
  # now call python script (unexpected relative path because job was called with working directory set as subdir of templogs so job can write output files) 
